# Programação Back-End 2025 - UNINOVE 

# Aula 1 -  Programação Back-end:
    ⠂Apresentação da Disciplina.

# Aula 2 -  INTRODUÇAO AO PHP
    ⠂O que é PHP
    ⠂Servidor Web
    ⠂Distribuição PHP
    ⠂Desenvolvendo em PHP(sites e plataformas)
       ▸ Wamp,Xampp

# 1 - Código Inicial (index.php, pg1.php)
    ⠂Estrutura Padrão HTML, tags.
    ⠂Sintaxe, uso de ponto e vírgula.
    ⠂Comando de saídas: echo e print.
  
# 2 - Introdução do Codespace(conhecendo ambiente de dev em nuvem) (index.php,pg04_01.php, pg04_02.php):
    Introdução ao GitHub.
        ⠂Ligando e Deslignado Máquina. ./servidor
        ⠂Explicação da página Index.
        ⠂Criação de um novo arquivo.
        ⠂Criação de link para página específica, a partir da página principal(index).
        ⠂Adicionando comentários.

# 3 - Tipos de Dados (aula05_01.php e aula05_02.php)
    ⠂ Declaração de uma Variável.
    ⠂ Tipos de Variáveis.
    ⠂ Como ocorre as alterações de uma variável.
    ⠂ Nomeando Corretamente uma variável.
    ⠂ Elemento de Teste 
        ▸var_dump
    ⠂ Definindo uma Constante.
        # Lição de casa : Manipulação de Dados e Concatenação 
            ▸ ldc01_01.php e ldc01_02.php

# 4 - Criando Formulário ( aula06_01.php  e aula06_02.php)
    ⠂ Tag Form
    ⠂ Method(características do GET e POST) e Action
    ⠂ Input  e tipos de Input
        # lição de de casa: 
            ▸ criar um formulário para cálculo de média.

# 5 - Operadores (aula07_01.php, aula07_02.php e aula08_01.php)
    ⠂ Operadores Aritiméticos
    ⠂ Operadores de Atribuição
    ⠂ Operadores de String
    ⠂ Operadores de Incremento e decremento
    ⠂ Opradores de Comparação
    ⠂ Operadores Lógicos
    ⠂ Parte 2 da explicação dos operadores
       ▸ Relacionais (comparação)
       ▸ Lógicos


# 6 - Estrutura Condicional(aula09_01.php,aula09_02.php)
    ⠂ If e Else
    ⠂ Operador ternário
    ⠂ Md5(método simpes e obsolete de criptografar dados)
    ⠂ switch/Case

# 7 - Estruturas de Repetição (aula10_01.php)
    ⠂ While
    ⠂ do While
    ⠂ for

# 8 - Arrays (aula11_01.php)
    ⠂ Array Indexado
    ⠂ Arrays Associativo
    ⠂ Arrays Multidimencional

# 9 - Foreach (aula12_01.php)
    ⠂ Estrutura de Repetição Foreach

# 10 - Sessão (aula13_01.php a aula13_menu.php)
    ⠂ Armazenamento dos dados
    ⠂ Remoção de dados
    ⠂ Finalizando Sessão



# INTREGRAÇAO COM BANCO DE DADOS(
    ⠂Configuração, Conexão, Criação Tabela
    ⠂Cadastro,Leitura, Atualização, Delete (CRUD)

# Lição de Casa - Verificação de Ímpar ou Par(ldc_imPar.php)
    Cálculo e análise do resto da divisão para determinar ser um valor é par ou ímpar.



    


